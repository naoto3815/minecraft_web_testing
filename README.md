...
### B. スマホで操作する (📱 リモートコントローラー)

スマホをゲームパッド代わりにして、PC画面のゲームを操作できる。
こちらは Node.js のサーバーを使う (追加インストール不要・標準モジュールのみ):
...
1. 起動すると `★ PCで開く: http://192.168.x.x:8000/` のような URL が表示される。
   **localhost ではなくこの IP の URL** を PC のブラウザで開く。
2. ゲーム画面右上の「📱 スマホで操作」を押すと、QRコードと部屋コードが出る。
3. **PCと同じ Wi-Fi** に繋いだスマホでQRを読み取る (またはコードを入力)。
4. スマホ画面の左で移動、右で視点、ボタンでジャンプ・破壊・設置・ブロック選択。

> 仕組み: `server.js` が静的ファイル配信と WebSocket リレーを兼ねる。
> スマホ(`controller.html`)の入力を同じ「部屋コード」のPC画面へ中継している。
> 外部サーバーには繋がず、家庭内 LAN だけで完結する。
...
- `index.html` — HTML / CSS / JavaScript がすべて入った1ファイル
  ...
  - **JavaScript部分** (`<script type="module">`): Three.jsを使った3D描画とゲームロジック
    - 末尾の `setupRemoteControl()` がスマホ入力をゲーム状態に反映する
- `server.js` — ローカルサーバー兼 WebSocket リレー（スマホ操作用・依存パッケージなし）
- `controller.html` — スマホ側のコントローラー画面（仮想ジョイスティック＋ボタン）
